...
def test_run_stage3_builds_change_records(sqlite_engine, tmp_path):
    config = load_competition_llm_config(Path("configs/competition_llm.yaml"))
    orchestrator = CompetitionLLMOrchestrator(
        engine=sqlite_engine,
        llm_orchestrator=StubLLM(),
    ...
    with sqlite_engine.begin() as conn:
        ...
        conn.execute(
            text(
                """
                INSERT INTO bi_amz_comp_delta (
                    scene_tag, base_scene, morphology, marketplace_id,
                    window_id, my_asin, opp_type, week_w0, week_w1, my_parent_asin,
                    d_price_net, d_rank_score, d_social_proof, d_content_score,
                    badge_change, d_price_gap_leader, d_price_index_med, d_rank_pos_pct,
                    d_content_gap, d_social_gap, delta_pressure
                )
                VALUES (
                    :scene_tag, :base_scene, :morphology, :marketplace_id,
                    :window_id, :my_asin, :opp_type, :week_w0, :week_w1, :my_parent_asin,
    ...
    result = results[0]
    assert result.context["week_w0"] == "2025-W02"
    assert result.context["week_w1"] == "2025-W01"
    ...
    gap_record = next(record for record in price_dim.records if record.metric == "d_price_gap_leader")
    assert gap_record.leader_changed is True
    assert gap_record.value_w0 == pytest.approx(5.0)
    assert gap_record.value_w1 == pytest.approx(3.0)
    assert gap_record.change_summary == "与 leader 的净价差拉大 2.00"

    median_record = next(record for record in price_dim.records if record.opp_type == "median")
    assert median_record.direction == "down"
    assert median_record.change_summary == "相对中位数的价格指数下降 0.05"

    confidence_dim = next(dim for dim in result.page_dimensions if dim.lag_type == "confidence")
    assert confidence_dim.total_changes == 1
    pressure_record = confidence_dim.records[0]
    assert pressure_record.change_summary == "相对压力上升 0.20"
